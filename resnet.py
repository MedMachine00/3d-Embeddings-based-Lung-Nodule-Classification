...
# -*- coding: utf-8 -*-
"""
Created on Sat Dec 14 20:21:17 2024
...
class ResNet(nn.Module):
    def __init__(self, block, layers, sample_input_D, sample_input_H, sample_input_W,
                 shortcut_type='B', no_cuda=False, embedding_dim=2048):
        super(ResNet, self).__init__()
        self.inplanes = 64
        self.no_cuda = no_cuda
        self.conv1 = nn.Conv3d(
            1,  # Single channel input for CT
            64,
            kernel_size=7,
            stride=(2, 2, 2),
            padding=(3, 3, 3),
        ...
        self.embedding_fc = nn.Linear(512 * block.expansion, embedding_dim)
    ...
    def forward(self, x):
        ...
        x = self.avgpool(x)
        x = torch.flatten(x, 1)
        embeddings = self.embedding_fc(x)
